...
- **`Lead.clientId` is a `String`**: not a FK to `Client`. The duplicate email check is now per-client (composite unique index on `email + client_id`), fixed in LSB-19. Phase 2 CLIENT-role auth scoping (`AuthPrincipalUtil.resolveStringClientIdForCaller`) relies on `Lead.clientId == Long.toString(client.id)` — fragile; see LSB-89 for the FK refactor design.
- **Flyway (LSB-36, PR #19)**: `ddl-auto` is `validate`; schema changes require a `VX__description.sql` file in `src/main/resources/db/migration/`. `baseline-on-migrate=true` so V1 is recorded as applied without running on existing prod DBs — when adding Flyway live, V2-V4 had to backfill columns that prior `ddl-auto:update` runs had created. Adding `NOT NULL` columns still needs a DB default or nullable, plus the migration file. V5 added `users.client_id` for Phase 2 CLIENT role.
- **CLIENT role auth (PORTAL-82/83/84)**: `User.clientId` is `Long` nullable. CLIENT JWTs include a `clientId` claim; ADMIN tokens don't. `JwtAuthenticationFilter` populates a `CustomUserPrincipal` (extends Spring `User`) carrying role + clientId. Read it via `AuthPrincipalUtil.currentClientId()` / `currentRole()`. For endpoints with a `clientId` filter, wrap the param in `resolveClientIdForCaller(Long)` (or `resolveStringClientIdForCaller(String)` for `Lead.clientId`); for get-by-id endpoints, call `assertCanAccessClient(Long)` after loading the entity. Admin-only endpoints stay `@PreAuthorize("hasRole('ADMIN')")`; CLIENT-accessible ones widen to `hasAnyRole('ADMIN', 'CLIENT')`.
