...
- **Rate limiter (LSB-37 / LSB-167)**: Bucket4j-on-Redis via Lettuce as of LSB-167. Local dev needs the Redis service from `docker-compose.dev.yml` running (`docker compose -f docker-compose.dev.yml up -d redis`). Prod reads `REDIS_URL` from Coolify. Buckets now survive restarts and stay coherent across replicas. Rapid test submissions still need `sleep 2` between requests (real refill, not test-side cache reset). Three key namespaces in Redis: `rl:api:<apiKey>`, `rl:public:<endpoint>:<ip>`, `rl:login:<email>`.
- **Em dash in bash heredoc**: Unicode `—` in curl JSON payloads causes 400 "Failed to read request". Use ASCII `-` instead.
- **Schema drift audit (LSB-90 / LSB-168)**: `SchemaAuditRunner` (`@Profile("prod")`) runs on startup, compares Hibernate entity column mappings against `information_schema.columns`. Default is **abort on drift** as of LSB-168 — an entity field shipped without a matching Flyway migration will crash prod boot loudly instead of buried-WARN. Set `NEUROFORGED_SCHEMA_AUDIT_FAIL=false` in Coolify if a future deploy genuinely needs to land a migration before the entity that backs it.
